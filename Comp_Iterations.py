...
BER_SIC_main = np.zeros((num_iter, SNRdB_range.shape[0]))
...
for dd in range(0, SNRdB_range.shape[0]):
    ...
    BER_SIC_block = np.zeros((num_iter, num_blocks))
    ...
    for bb in tqdm(range(0, num_blocks)):
        ...
        BER_SIC_vector = np.zeros((num_iter, block_size))
...
plt.grid(True)
plt.title('Iterative SIC Tx10Rx10')
plt.savefig('./figures/debug-Tx10Rx10.png')